...
{:.changelog}
*  22 Nov 2022: add note that step 1 might also help with address calculation issues; add possibility of simplify address calculations to "extra optimizations"
*  28 Nov 2022: update part 4a hints to avoid implying that you can store directly into `dst` and instead refer to strategy seen in part 3
*  5 Dec 2022: after discussing packus, note explicitly that we can store directly from the vector into pixels when we've done the conversion.
...
3.  Edit `smooth.c` to make the `who_t` structure to include the your name and computing ID.
    ...
    [more detailed result viewer]({{site.submit_site}}smoothviewer)
...
5.  Test your implementation to make sure it is correct and to see what average speedup it gets.
    ...
    2.  If you submit `smooth.c` to [the submission site]({{site.submit_site}}){:target="_top"}, we
        will time it on our machine and post the results
        [here]({{site.submit_site}}smoothviewer/){:target="_top"}.
...
6.  For the checkpoint submission, you must get a speedup of at least 2.50 ***on our testing machine***.
    ...
    There is a seperate submission form for the checkpoint and final submission on the submission site.
    We will compute
...
the corners and edges. When you do this, you'll avoid performing comparisons for most of the pixels that the current code perform. Simplifying the loop may also make it easier for the compiler or, in later steps, you to perform other optimizations.

(In particular, the compiler has optimizations that simplify address calculations by taking advantage of addresses being related between iterations of the loops. For example, indices differing by 1 between iterations can be computed with a single addition rather than by redoing a bunch of multiplications and additions. The compiler is usually quite good at doing these optimizations for you, but I have found that sometimes the compiler won't do these optimizations well when there are conditionals.)
...
Near the edges of the image, it may be the case that you don't have a group of four pixels, but only two to compute. In this case, you can use the same code and still compute values for four pixels, but ignore the last two of them when it comes time to write them to the destination image. If you convert the vector into array of shorts, as a typical
part 3 solution would, this means that you'll need to handle a variable number of those array elements being useful (16 (4 pixels worth) or 8 (2 pixels worth), depending
on the locatoin in the image).
...
from two 256-bit vectors and returns a single 256-bit vector of 8-bit integers. We should then be able to store
from the vector directly into the pixels (rather than copying component-by-component).
...
*  Simplfiying address calculations: *sometimes* the compiler will generate unnecessarily complicated address calculation code. This can include
   ...
   *  compute the address of one element by adding to the address of a previous one as in
...
You can view detailed results [here]({{site.submit_site}}smoothviewer/), which include the times for each version you submitted.
